...
data_root1 = pathlib.Path('C:\\MLProjects\\venv2\\awesome')
...
X = tf.placeholder(tf.float32, [batch_size, num_input])
images = tf.reshape(X, [-1, 100, 100, 1])
#создание сверточного слоя (применяет сверточные фильтры):
conv1_logits = tf.nn.conv2d(images, ae_weights['conv1'], strides=[1, 2, 2, 1], padding='SAME') + ae_weights['b_conv1']
...
optimizer = tf.train.AdagradOptimizer(learning_rate).minimize(total_cost)
