...
data_root1 = pathlib.Path('C:\\MLProjects\\emptyYOBA3')
...
def encoder(x):
    encod_h1_logits = tf.nn.conv2d(x, ae_weights['encod1'], strides=[1, 1, 1, 1], padding='SAME') + ae_weights['b_encod1']
    encod1 = tf.nn.relu(encod_h1_logits)
    encod_h1_maxpool = tf.nn.max_pool(encod1, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
    encod_h2_logits = tf.nn.conv2d(encod_h1_maxpool, ae_weights['encod2'], strides=[1, 1, 1, 1], padding='SAME') + ae_weights['b_encod2']
    encod2 = tf.nn.relu(encod_h2_logits)
    encod_h2_maxpool = tf.nn.max_pool(encod2, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
    encod_h3_logits = tf.nn.conv2d(encod_h2_maxpool, ae_weights["encod3"], strides=[1, 1, 1, 1], padding="SAME") + ae_weights["b_encod3"]
    encod3 = tf.nn.relu(encod_h3_logits)
    encod_h3_maxpool = tf.nn.max_pool(encod3, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
    return encod_h3_maxpool
